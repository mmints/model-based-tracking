...
int main(int argc, char **argv)
{
    ...
    while( !glfwWindowShouldClose(window))
    {
        ...
        if (zed.retrieveImage(gpuLeftImage, VIEW_LEFT, MEM_GPU) == SUCCESS) {
            cudaArray_t ArrIm;
            kernel::simpleRedDetector(gpuLeftImage.getPtr<sl::uchar4>(MEM_GPU), outImage.getPtr<sl::uchar4>(MEM_GPU), 140, width, height, gpuLeftImage.getStep(MEM_GPU));
            //kernel::simpleRedDetector(gpuLeftImage.getPtr<sl::uchar4>(MEM_GPU), outImage.getPtr<sl::uchar4>(MEM_GPU), width, height, gpuLeftImage.getStep(MEM_GPU));
