...
mt::ParticleGenerator::ParticleGenerator(int particle_count, int particle_width, int particle_height) {
    srand(time(0)); // Set a seed for evey new instantiated ParticleGenerator
    if ((int)std::pow((int)std::sqrt(particle_count), 2) != particle_count) {
        throw std::invalid_argument("[mt::ParticleGenerator::ParticleGenerator] Invalid Particle Count. "
                                    "Make sure that it is a result of a square operation.");
    ...
    CVK::Sphere *sphere = new CVK::Sphere();
    CVK::Material *mat_red = new CVK::Material(1.f, RED);
    m_model = new CVK::Node("sphere");
    m_model->setMaterial( mat_red);
    m_model->setGeometry( sphere);
...
void mt::ParticleGenerator::updateParticles(std::vector<Particle> &particles) {
    glm::vec3 translation;      // TODO: Does this take additional memory space or computation time?
    glm::vec3 rotation_angles;
...
void mt::ParticleGenerator::updateParticles(std::vector<Particle> &particles, float translation_deviation, float rotation_deviation) {
    glm::vec3 translation;      // TODO: Does this take additional memory space or computation time?
    glm::vec3 rotation_angles;

    for (int i = 0; i < m_particle_count; i++) {
        translation = glm::gaussRand(particles[i].getTranslation(), glm::vec3(translation_deviation));
        rotation_angles = glm::gaussRand(particles[i].getRotation(), glm::vec3(rotation_deviation));
        particles[i].setModelMatrix(translation, rotation_angles);
    }
}